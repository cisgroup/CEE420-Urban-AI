...
3. VS Code asks which Python environment to use. Pick the one marked *Recommended*, whose path is =/opt/venv/bin/python=. It is named after the course, =urban-ai=, and it is the only one inside the container; anything reading =/usr/local/bin/python= is the wrong choice.
